...
## ⚠️ Common Gotcha: Function Wrappers & oklch()

While using CSS variables is the standard, you must be careful about **color space functions**.

**The Mistake:**

```tsx
const color = `hsl(var(--chart-1))`; // This will render BLACK
```

**Why it fails:**
Your project uses **oklch()** (the modern CSS color standard) in `index.css`. If you wrap an `oklch` value inside an `hsl()` function, the browser's CSS parser breaks and defaults the color to black.

**The Fix:**
Always use the variable directly or via a standard CSS-compliant mapping:

```tsx
const color = `var(--chart-1)`; // Correct
```
